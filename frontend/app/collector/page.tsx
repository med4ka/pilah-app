...
export default function CollectorDashboard() {
  ...
  const scanArea = async () => {
    ...
    try {
      ...
      const data = await getPendingPickups(token).catch(() => [
        { id: 'dummy-1a2b3c4d', latitude: 0, longitude: 0 },
  ...
  const handleComplete = async () => {
    ...
    setIsProcessing(true)
    try {
      await completePickup(token, activeOrder.id.toString()).catch(() => console.log('Simulasi selesai order'))
      setActiveOrder(null)
      ...
      <main className="w-full max-w-md h-[100dvh] sm:h-[850px] bg-white flex flex-col relative overflow-hidden sm:rounded-[2.5rem] shadow-2xl border border-zinc-100">
        ...
        <div className="flex-grow overflow-y-auto px-6 pt-2 pb-32 scrollbar-hide">
          ...
          {message && activeTab === 'radar' && (
            <div className={`mb-6 p-4 rounded-2xl flex items-start gap-3 text-sm font-bold border animate-in fade-in slide-in-from-top-2 ${message.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 shadow-[0_4px_12px_rgba(16,185,129,0.1)]' : 'bg-red-50 text-red-700 border-red-100'}`}>
              {message.type === 'success' ? <CheckCircle2 className="shrink-0 mt-0.5" size={18} /> : <AlertCircle className="shrink-0 mt-0.5" size={18} />}
              <p className="leading-snug text-xs">{message.text}</p>
            </div>
          )}
                 ...
                 {activeOrder ? (
                   <div className="bg-white p-6 rounded-[2rem] border border-emerald-100 shadow-[0_10px_30px_-10px_rgba(16,185,129,0.2)] animate-in slide-in-from-bottom-4">
                      ...
                      <p className="text-sm font-semibold text-zinc-500 mb-6 flex items-center gap-2 bg-zinc-50 px-3 py-1.5 rounded-lg w-fit">
                        <Navigation size={14} className="text-emerald-500"/> ID: {activeOrder.id.toString().substring(0,8)}
                      </p>
                      <button onClick={handleComplete} disabled={isProcessing} className="w-full py-4 rounded-[1.25rem] font-bold flex items-center justify-center gap-2 transition-all bg-emerald-500 text-white hover:bg-emerald-600 active:scale-[0.98] disabled:opacity-70 shadow-md shadow-emerald-500/20">
                   ...
                   <div className="bg-zinc-900 p-6 rounded-[2rem] shadow-xl animate-in slide-in-from-bottom-4 border border-zinc-800 relative overflow-hidden">
                      ...
                      <div className="flex items-center justify-between mb-6 relative z-10">
                         <div>
                           ...
                           <p className="font-black text-white text-lg mt-0.5">ID: {selectedPickup.id.toString().substring(0,8)}</p>
                         </div>
            ...
            <div className="animate-in fade-in duration-300">
              ...
              {isLoadingHistory ? (
                ...
                <div className="space-y-3">
                  <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3 px-1">Riwayat Perjalanan</p>
            ...
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              {userData ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  <ShieldCheck size={56} className="text-zinc-200 mb-6" strokeWidth={1.5} />
                  <p className="text-zinc-500 font-medium mb-6">Silakan masuk untuk melihat profil mitra.</p>
                  <button onClick={openAuthModal} className="bg-zinc-900 text-white px-8 py-3.5 rounded-[1.25rem] font-bold shadow-md hover:bg-zinc-800 transition-colors active:scale-95">Masuk Sekarang</button>
                </div>
              )}
            </div>
          )}
