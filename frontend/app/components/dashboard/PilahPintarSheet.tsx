...
import { X, BookOpen, CheckCircle2, Camera, Sparkles, RefreshCcw } from 'lucide-react'
...
if (!isPilahPintarOpen) return null
   ...
   const categories = [
      { id: 1, name: 'Botol Plastik (PET)', image: '/botol.png', status: 'Bisa Didaur Ulang', ok: true, desc: 'Cuci bersih dan remukkan sebelum dibuang.' },
      { id: 2, name: 'Kardus & Kertas', image: '/kardus.png', status: 'Bisa Didaur Ulang', ok: true, desc: 'Pastikan kering dan tidak berminyak.' },
      { id: 3, name: 'Kaca & Beling', image: '/jar.png', status: 'Bisa Didaur Ulang', ok: true, desc: 'Pisahkan dari sampah lain agar aman.' },
   ]
               ...
               <div className="space-y-6">
                  ...
                  <div>
                     ...
                     <div className="grid gap-3">
                        {categories.map(cat => (
                        <div key={cat.id} className="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm flex items-center gap-4 hover:border-zinc-300 transition-colors">

                           <div className="bg-zinc-50 w-14 h-14 flex items-center justify-center rounded-xl shrink-0 border border-zinc-100 overflow-hidden p-2">
                              <img src={cat.image} alt={cat.name} className="w-full h-full object-contain drop-shadow-sm group-hover:scale-110 transition-transform" />
                           </div>
