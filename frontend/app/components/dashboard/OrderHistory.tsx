    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-300 pb-8">
      ...
      <div className="flex flex-col gap-4">
        {history.length === 0 ? (
          ...
          history.map((order) => {
            const plastic = order.plastic_weight || 0;
            const cardboard = order.cardboard_weight || 0;
            const glass = order.glass_weight || 0;
            const totalKarma = (plastic * 10) + (cardboard * 7) + (glass * 15);

            return (
              <div 
                key={order.id} 
                onClick={() => setSelectedOrder({ ...order, totalKarma, plastic, cardboard, glass })}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-zinc-900 tracking-tight">
                        ...
                        order.status === 'VERIFYING' ? 'Verifikasi Timbangan' :
                  ...
                  <div className="text-right">
                    <p className={`font-black ${order.status === 'COMPLETED' ? 'text-emerald-600' : 'text-zinc-300'}`}>
                      {order.status === 'COMPLETED' ? `+${totalKarma} Karma` : '0 Karma'}
                    </p>
        ...
        <div className="fixed inset-0 z-50 flex justify-center items-end sm:items-center bg-zinc-900/40 backdrop-blur-md animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-md bg-white rounded-t-[2.5rem] sm:rounded-[2rem] p-6 sm:p-8 shadow-2xl animate-in slide-in-from-bottom-full sm:zoom-in-95 duration-300 ease-out">
            ...
            <div className="space-y-4">
              ...
              {(selectedOrder.status === 'COMPLETED' || selectedOrder.status === 'VERIFYING') && (
                <div className="py-3 border-b border-zinc-50 space-y-2">
                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1 block">Rincian Setoran</span>
                  <div className="flex justify-between text-sm font-bold text-zinc-700">
                    <span className="flex items-center gap-1.5">🥤 Botol Plastik</span>
                    <span>{selectedOrder.plastic} kg</span>
                  </div>
                  <div className="flex justify-between text-sm font-bold text-zinc-700">
                    <span className="flex items-center gap-1.5">📦 Kardus/Kertas</span>
                    <span>{selectedOrder.cardboard} kg</span>
                  </div>
                  <div className="flex justify-between text-sm font-bold text-zinc-700">
                    <span className="flex items-center gap-1.5">🫙 Botol Kaca</span>
                    <span>{selectedOrder.glass} kg</span>
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-between items-center py-3 border-b border-zinc-50">
                ...
                <span className="text-lg font-black text-emerald-600">
                  {selectedOrder.status === 'COMPLETED' ? `+${selectedOrder.totalKarma} Points` : '0 Points'}
                </span>
              </div>
