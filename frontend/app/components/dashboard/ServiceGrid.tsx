...
export default function ServiceGrid({ onJemputClick, isSearching, disabled }: ServiceGridProps) {
  const { openPilahPintar, openDropPoint, openMoreSheet } = usePilahStore()
  ...
  const services = [
    ...
    { 
      id: 'more', name: 'Lainnya', icon: null, active: true, action: openMoreSheet, 
      style: 'bg-zinc-50 text-zinc-600 border-zinc-200 hover:bg-zinc-100 hover:border-zinc-300' 
    },
  ];
            ...
            >
              {svc.id === 'jemput' && isSearching ? (
                ...
                <div className="flex gap-1.5 opacity-60">
                  {[...Array(3)].map((_, i) => <div key={i} className="w-1.5 h-1.5 rounded-full bg-zinc-600" />)}
                </div>
              )}
            </button>
