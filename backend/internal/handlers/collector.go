...
// ==========================================
// 📍 VIBE CODE: COLLECTOR ENGINE (MVP VERSION)
// ==========================================

// GetPendingPickups mengambil orderan yang belum di-handle (Bypass GPS Strict Params)
func GetPendingPickups(c *fiber.Ctx) error {
	start := time.Now()
	var pickups []models.Pickup

	// ⚡ PERFORMANCE CONSTRAINT: Tarik langsung dari DB tanpa filter radius GPS untuk MVP
...
// AcceptPickup mengubah status order dari PENDING menjadi ACCEPTED (UUID Support)
func AcceptPickup(c *fiber.Ctx) error {
	// Tangkap ID sebagai String murni (UUID)
	...
	// ⚡ SECURITY CONSTRAINT: Cek orderan dengan UUID string (Mencegah SQL Injection via ORM)
